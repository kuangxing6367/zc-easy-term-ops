...
# ------------------------------------------------------------
# 备份文件（时间戳副本，安全修改配置前的统一入口）
# 参数：$1 源文件路径
# 输出：备份文件路径（失败时输出空）
# 返回：0=成功 1=失败（文件不存在或复制失败）
# ------------------------------------------------------------
backup_file() {
    local src="$1"
    local dest=""
    if [[ ! -f "${src}" ]]; then
        log_warning "备份跳过（文件不存在）: ${src}"
        return 1
    fi
...
try() {
    ...
    if "$@"; then
        ...
        log_warning "${desc} 失败，继续执行"
        return 1
    fi
}

# ------------------------------------------------------------
# 统一错误终止封装：执行命令，失败时记录错误并退出
# 参数：$1 操作描述  $@ 命令及参数
# 返回：无（失败时 exit 1）
# ------------------------------------------------------------
require() {
