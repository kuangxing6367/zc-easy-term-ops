...
### 修复（文件管理器鼠标两段式 / 主菜单滚动校准与转义泄漏）

- **文件管理器鼠标交互与主菜单一致（modules/20_file_manager.sh）**
  - 原交互：列表项需「双击」才进入目录/打开文件，单击只移动光标——用户易误以为"目录进不去"
  - 改为两段式：**单击列表项 = 选中（移动光标）；再次点击同一项 = 确认进入目录（或打开文件操作菜单）**，与主菜单一致
  - 进入目录 / 返回上级时重置两段式状态，避免跨目录误判"再次点击"
- **主菜单滚动校准增强 + 转义残留清理（core/menu.sh）**
  - `_tui_finalize_rows` 依赖 DSR(`ESC[6n`) 查询光标行校准滚动偏移；若终端不响应 DSR，原逻辑按"未滚动"处理 → 内容滚动后整屏点击错位（表现为"某排点不动"）
  - 增强：DSR 查询失败时改用 `stty size` 终端行数估算光标行（`min(内容行, 屏幕行)`），滚动后点击仍能精确命中
  - 查询结束后清空 DSR 响应残留字节，防止残留转义序列在后续被当普通文本回显（"点击后出现乱码/原始序列"）
- **自动化测试（tests/run_tests.sh）**：新增文件管理器两段式断言（首次点击仅选中、同项再点确认、确认后重置状态）（104/104 通过）

## [1.5.2] - 2026-08-31

### 修复（主菜单双列点击 / 文件管理器 / 单实例锁）

- **修复主菜单右列(14-27)点不到 + 双列排版错位（core/menu.sh）**
  - 根因 1：点击分界硬编码 `col<40`，而渲染左列名称 `%-20s` 按字节/字符填充，中文名（每字 2 列）实际占满更多列，右列真实起始列与 40 不符 → 点右列被误判成左列
  - 根因 2：`%-20s` 固定宽度不按显示宽度对齐，中文菜单名导致左右列挤压错位、不美观
  - 修复：`_ui_module_list` 改为按显示宽度布局（左列名称截断到 22 列、右列从第 30 列开始，`_TUI_LCOL_W`/`_TUI_RIGHT_COL` 记录列常量），`_tui_row_to_opt` 点击分界改为与渲染一致的 `_TUI_RIGHT_COL`；宽度函数 `fm_str_w`/`fm_str_clip` 提升到 `core/utils.sh` 公共复用
- **文件管理器（modules/20_file_manager.sh）**
  - 新增 `../` 上级目录入口：列表顶部固定 `..` 项（`fm_cur_is_parent` 识别），点击/回车即返回上级
  - 危险操作防护：`..` 项不可标记(Space)/复制/剪切/删除/改名/改权限/编辑，避免误操作整个父目录
  - 渲染提速：每个文件 stat 由 2 次合并为 1 次，大目录下响应更流畅
- **单实例锁改为 PID 锁 + 过期检测（core/main.sh）**
  - 原 flock 锁在进程异常退出（如 SSH 断开被杀）后锁文件残留，易造成"断开后锁文件还在"
  - 改为锁文件记录 PID：下次启动若记录的 PID 已不存在则视为过期锁自动清理后继续；存活实例才拒绝并发；正常退出删除锁文件。不再依赖 flock（保持 0 外部依赖）
- **自动化测试（tests/run_tests.sh）**：修复 `check_true`/`check_false` 在 `set -e` 传导下的子 shell 退出陷阱；新增双列分界、`../` 入口与防护、PID 锁 stale 检测断言（100/100 通过）
