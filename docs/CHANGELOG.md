...
- **README 全面重写**：突出项目主要特点（纯 Bash 零依赖、全 TUI 交互、内置 TUI 编辑器、AI 智能运维助手、TUI+CLI 双模式、安全加固、模块化可扩展），补充功能模块一览表与交互约定说明
- **修复提示堆叠（prompt 重复追加）**：菜单方向键/鼠标选中后刷新输入提示时改用 `\r\033[K`（回车 + 清当前行）覆盖旧提示，避免旧 prompt 残留堆叠（"提示一直增加 / 自动刷新命令"的体验问题）；首次进入提示保持原样
- 自动化测试 170/170 通过

## [1.5.8] - 2026-09-01

### 新增（全菜单统一 TUI 方向键导航 + 提示更新）

- **方向键导航（主菜单 + 全部 26 个子菜单 + 插件菜单统一生效）**
  - `_tui_read_event` 新增 CSI 方向键解析（`ESC[A/B/C/D` → `key:up/down/right/left`）
  - `get_user_choice`（主菜单/子菜单共用入口）新增方向键处理：↑/← 上移、↓/→ 下移，在当前渲染选项集合中移动选中项（复用 `_TUI_OPT_ROW_OPT` 行映射，选项去重并按数字升序保证导航顺序）
  - 选中项高亮（与鼠标首次点击同款），**回车或再点同一项确认进入**；滚轮翻页（主菜单分页）保持不变
  - 方向键、鼠标点击、数字键三种方式完全等价，交互统一
- **提示文案更新**：主菜单页脚、子菜单/插件标题栏、输入提示均改为"方向键/鼠标选中，回车或再点确认"，让用户知道可用方向键
- **自动化测试**：新增 12 项方向键断言（解析 + 选中移动 + 边界停留 + 与鼠标混合确认），170/170 通过

### CLI 说明（已具备，无需额外改动）

- 全部模块既支持交互式 TUI，也支持非交互 CLI 直达：`zetops --run 模块序号|短名 选项号`（如 `zetops --run 13 1` / `zetops --run ai_assistant 1`），另有 `--help` / `--version` / `--list` / `--backup`

## [1.5.7] - 2026-09-01

### 优化 / 修复（内置 TUI 文本编辑器）

- **界面改为 nano 风格布局**：顶部标题栏（程序名 版本 文件路径 修改/只读标记）+ 无行号编辑区 + 底部状态行（行列/行数/只读）+ 底部两行快捷键栏（`^G 帮助 ^O 保存 ^X 退出 M-U 撤销 M-E 重做` / 操作提示），光标可见（不再隐藏）
- **新增撤销 / 重做**：`Alt+U` 撤销、`Alt+E` 重做（nano 官方键位）；支持连续多步撤销/重做，编辑操作前自动入栈（上限 100 步），撤销后新编辑自动清空重做栈；`ed_load` 打开新文件时自动清空撤销栈
- **弃用 Ctrl+Z/Ctrl+Y 做撤销**：`Ctrl+Z` 在 shell 中本就是"挂起任务到后台"（SIGTSTP），现明确丢弃（不挂起、不误插入），与 nano 行为一致
- **修复单按 Esc 卡死**：二级/三级按键读取加 1 秒超时，单独按 Esc 立即退出（此前会阻塞等待后续字节）
- **修复快照恢复元数据错位**：撤销/重做恢复时 `ed_restore` 非贪婪解析行与光标元数据（此前贪婪匹配导致重做后光标行错位到下一行）
- 帮助页（`^G`）加入撤销/重做说明
- 自动化测试新增 14 项撤销/重做断言（含连续多步撤销、重做链、栈清空），158/158 通过

## [1.5.6] - 2026-09-01

### 新增（内置纯 Bash TUI 文本编辑器，零外部依赖）

- **内置 TUI 文本编辑器（新增 core/editor.sh）**
  - 完全由纯 Bash 实现，**不依赖 nano/vim/vi/ed** 等任何外部编辑器（符合"0 依赖"约束）
  - 功能：方向键/Home/End/PageUp/PageDown 光标移动、插入字符、退格、Delete、回车拆行、行首/行尾自动合并上一行/下一行
  - **完整支持中文等 UTF-8 多字节字符**：左右移动、退格、删除均按"完整字符"处理（不会切坏多字节字符），宽字符按 2 列计算（复用 fm_str_w/fm_str_clip）
  - 鼠标点击定位光标（SGR 1000/1006 协议，滚轮/移动/悬停等指针信息仍丢弃）
  - 行号列 + 标题栏 + 状态栏（文件名/行列/未保存标记/只读标记），大文件滚动窗口跟随光标
  - 保存 Ctrl+S、退出 Ctrl+Q / Esc（有未保存修改时弹出 y/n/c 确认）、Ctrl+C 亦可退出
  - 只读文件自动检测并拒绝保存
- **文件管理器编辑优先用内置编辑器（modules/20_file_manager.sh）**
  - `fm_edit_file` 检测到 `tui_edit_file` 时优先调用内置编辑器；内置未完成时回退外部链 nano→vim→vi→ed
- **自动化测试（tests/run_tests.sh）**：新增 [21] 编辑操作纯逻辑块（加载/插入/退格/删除/回车/合并/移动/列换算/保存/只读，含中文多字节用例），28 项断言（144/144 通过）
...
- **丢弃鼠标指针信息（core/menu.sh）**
  - SGR/X10 鼠标协议中 btn≥32 的事件（移动/悬停/拖拽/滚轮）现在直接丢弃，不产生输入、不回显
  - 普通输入只接受可打印字符，控制字符（鼠标序列残留/功能键字节）静默丢弃——不再出现"点击后残留转义序列被当普通文本打印"
  - ESC 序列读取不完整时清空 tty 残留，防止拆分后的悬停序列污染后续解析
- **滚动校准弃用 DSR（core/menu.sh）**
  - `_tui_finalize_rows` 不再依赖 `ESC[6n`（XShell/部分终端响应慢或格式异常导致整屏错位），改用 `stty` 屏幕行数估算光标行（`min(内容行, 屏幕行)`），主菜单内容滚动后各排点击仍精确命中
- **渲染整屏一次输出（core/menu.sh + modules/20_file_manager.sh）**
  - 主菜单/子菜单/插件菜单与文件管理器均改为「整屏拼成单个大字符串 → 一次性 printf」
  - 大幅缩短渲染输出窗口：渲染期间鼠标事件不再在 tty 输入缓冲大量堆积（"某排点不动 / 悬停乱码"的根治）
- **文件管理器编辑回退链（modules/20_file_manager.sh）**
  - 编辑器选择回退链：nano → vim → vi → ed，缺失/异常（rc≥2）自动切换下一个，不再"不能编辑"
  - rc 0/1/130 视为用户正常完成（保存/放弃/中断）
  - 空参时优先编辑当前选中文件；操作菜单绑定当前选中文件并显示
- **自动化测试（tests/run_tests.sh）**：新增鼠标指针信息丢弃断言、编辑回退链 7 项断言（115/115 通过）
